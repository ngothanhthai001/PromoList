...
define([
    ...
    return uiCollection.extend({
        ...
        collapsibleCoupon: function (index){
            var classCouponTitle = ".coupon_"+index, classCouponContent = ".coupon_content_"+index;
            if($(classCouponTitle).hasClass("_show")){
                $(classCouponTitle).removeClass("_show");
                $(classCouponContent).removeClass("_show");

            }else{
                $(classCouponTitle).addClass("_show");
                $(classCouponContent).addClass("_show");
            }
            return this;
        },
        ...
        getDataRecordCustom: function(index){
            var recordsData, dataRecord = [], dataShowRecord = [];
            recordsData = _.findWhere(this.recordData() , {
                id: index
            });
            dataRecord.push(recordsData);

            this.getLabelsCustom().forEach(function (elem){
                dataRecord.forEach(function (record){
                    if(record && record[elem.name] && elem.name !== 'position'){
                        dataShowRecord.push(record[elem.name]);
                    }
                })
            });
           return dataShowRecord;
        },

        getLabelsCustom: function (){
            return this.labels();
        },
        sendDataTitle: function (index,recordId){
            var titleValue,descriptionValue,classTitle,classDescription ;
            classTitle = `input[name=coupon_title_${index}]`;
            classDescription =`input[name=coupon_description_${index}]`;
            titleValue = $(classTitle).val();
            descriptionValue = $(classDescription).val();
            if(recordId){
                var recordsData = _.findWhere(this.recordData() , {
                    id: recordId
                });
                (titleValue || titleValue === '') ? recordsData.coupon_title = titleValue:  "";
                (descriptionValue || descriptionValue === '') ? recordsData.coupon_description = descriptionValue :  "";
                this.recordData[index] = recordsData;
                this.recordData(this.recordData());
            }
        },
        getCouponTitle: function (index,recordId){
            var recordsData = _.findWhere(this.recordData() , {
                id: recordId
            });
            if(recordsData){
                return recordsData.coupon_title;
            }
        },
        getCouponDescription: function (index,recordId){
            var recordsData = _.findWhere(this.recordData() , {
                id: recordId
            });
            if(recordsData){
                return recordsData.coupon_description;
            }
        }
    });
});
